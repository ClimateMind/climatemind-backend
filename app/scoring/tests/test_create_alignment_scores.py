...
def test_get_rank_map():
    ...
    assert (actual_rank_map == expected_rank_map), "Rank map does not have expected values."

def test_get_alignment_map():
    data_map = {'achievement':(1, 5, 0.728),
    ...
    actual_alignment_map = {name:round(value, 3) for (name, value) in actual_raw_alignment_map.items()}
    assert (actual_alignment_map == expected_alignment_map), "Alignment map does not have expected values."

def test_get_max():
    data_map = {'achievement':0.728,
