...
@bp.route("/user/session_id", methods=["PUT"])
@cross_origin()
@jwt_required()
def update_session_id():
    ...
    try:
        ...
        old_session_id = scores.session_uuid
        session = (
            db.session.query(Sessions)
            .filter_by(session_uuid=old_session_id)
            .one_or_none()
        )
        feed = (
            db.session.query(ClimateFeed)
            .filter_by(session_uuid=old_session_id)
            .one_or_none()
        )
        analytics = (
            db.session.query(AnalyticsData)
            .filter_by(session_uuid=old_session_id)
            .one_or_none()
        )
    except:
