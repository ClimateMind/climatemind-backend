...
def store_post_code(post_code, session_id):
    """
    ...
    If the post code is valid, it is committed to the Sessions table alongside the session_id.
    """
    ...
    if post_code_valid:
        try:
            s = Sessions.query.filter_by(session_id=session_id).first()
        except:
